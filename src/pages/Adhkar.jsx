...
export default function Adhkar() {
  ...
  const increment = (item) => {
    ...
    if (next >= item.count) setDone(prev => ({ ...prev, [item.id]: true }));
  };
    ...
    <div className="fade-in pb-24 px-4 pt-6 max-w-md mx-auto">
      ...
      <div className="flex gap-2 mb-4">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={"flex-1 py-2 px-3 rounded-xl text-sm transition-all " + (tab === t.id ? "bg-gold text-night font-semibold" : "bg-panel text-muted border border-white/5")}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1">
          {items.map(i => <div key={i.id} className={"w-2 h-2 rounded-full " + (done[i.id] ? "bg-gold" : "bg-white/10")}/>)}
        </div>
      </div>
      ...
      <div className="space-y-3">
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className={"card transition-all " + (isDone ? "border-jade/30 bg-jade/5" : "")}>
              <div className="p-4">

                {/* TRADUCTION EN PREMIER */}
                <div className="mb-3">
                  <p className="text-sm text-soft leading-relaxed italic">{item.fr}</p>
                </div>

                {/* SOURCE */}
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs bg-gold/10 text-gold px-2 py-0.5 rounded-full">{item.source}</span>
                  {item.count > 1 && <span className="text-xs text-muted">{item.count}×</span>}
                  {isDone && <span className="text-xs text-jade-light">✓ accompli</span>}
                </div>

                {/* Bouton pour voir l arabe */}
                <button onClick={() => setExpanded(isOpen ? null : item.id)}
                  className="text-xs text-gold-dim underline mb-2 block">
                  {isOpen ? "Masquer arabe" : "Voir en arabe"}
                </button>

                {isOpen && (
                  <div className="border-t border-white/5 pt-3 space-y-2">
                    <p className="arabic text-xl text-gold leading-loose text-right">{item.ar}</p>
                    {item.phonetic && <p className="text-xs text-muted font-mono leading-relaxed">{item.phonetic}</p>}
                    {item.merit && <p className="text-xs text-jade-light bg-jade/10 p-2 rounded-lg">✦ {item.merit}</p>}
                  </div>
                )}
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex gap-2">
                    {count > 0 && !isDone && (
                      <button onClick={() => reset(item.id)} className="text-xs text-muted px-2 py-1">↺</button>
                    )}
                    {!isDone ? (
